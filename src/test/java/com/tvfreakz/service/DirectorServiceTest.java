...
public class DirectorServiceTest {
    ...
    public void testFindAll() throws Exception {
        ...
        when(directorRepositoryMock.findAll(directorSort)).thenReturn(
                Arrays.asList(TestUtil.DIRECTORS));

        List<Director> directorList = directorService.findAll();

        assertEquals("Director list is the wrong size: ",
                TestUtil.DIRECTORS.length, directorList.size());
        ...
        assertEquals("Jean-Pierre Jeunet", directorList.get(2)
                .getDirectorName());
