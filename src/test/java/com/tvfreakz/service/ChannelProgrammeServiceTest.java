...
public class ChannelProgrammeServiceTest {
  ...
  public void testFilterChannelProgrammes() {
    ...
    when(channelProgrammeRepositoryMock.filterChannelProgrammes(any(String.class), any(String[].class), any(String[].class),
        any(DateTime.class), any(DateTime.class), any(Boolean.class), any(Boolean.class), any(Boolean.class))).thenReturn(Arrays.asList(channelProgrammeShowings));
    
    List<ChannelProgramme> channelProgrammeList = channelProgrammeService.filterChannelProgrammes(searchDTO);
    
    assertEquals("Channel Programme List is of the wrong size", 1, channelProgrammeList.size());
    assertEquals("Aliens", channelProgrammeList.get(0).getProgramme().getProgTitle());
  }
