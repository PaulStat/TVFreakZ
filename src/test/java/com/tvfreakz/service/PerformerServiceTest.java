...
public class PerformerServiceTest {
    ...
    public void testFindByPerformerIdWhenPerformerIsFound() throws Exception {
        when(performerRepositoryMock.findByPerformerId(1L)).thenReturn(
                performer);
    ...
    public void testFindAll() throws Exception {
        ...
        when(performerRepositoryMock.findAll(sort)).thenReturn(
                Arrays.asList(TestUtil.PERFORMERS));

        List<Performer> performerList = performerService.findAll();

        assertEquals("Performer list is of the wrong size: ",
                TestUtil.PERFORMERS.length, performerList.size());
        assertEquals("Sigourney Weaver", performerList.get(0)
                .getPerformerName());
