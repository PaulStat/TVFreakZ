...
public class PerformerBuilderTest {
  ...
  private Set<Programme> programmes = new HashSet<>(Arrays.asList(new Programme[]{aliens}));
  
  
  @Before
  public void setup() {
    expected = new Performer(performerId, performerName, programmes);
  }
  
  @Test
  public void testPerformerBuilderBuildObjectsShouldBeEqual() {
    Performer actual = new Performer.Builder().withPerformerId(performerId).withPeformerName(performerName).withProgrammes(programmes).build();
  ...
  public void testPerformerBuilderBuildObjectsShouldNotBeEqual() {
    Performer actual = new Performer.Builder().withPerformerId(performerId).build();
