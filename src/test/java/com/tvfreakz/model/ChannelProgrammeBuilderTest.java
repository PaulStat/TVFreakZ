...
public class ChannelProgrammeBuilderTest {
    ...
    private Channel channel = new Channel.Builder().withChannelId(1L)
            .withChannelName("BBC1").build();
    ...
    @Before
    public void setup() {
        expected = new ChannelProgramme(channelProgrammeId, channel, programme,
                episode, repeat, newSeries, premiere, choice, subtitles,
                deafSigned, starRating, startTime, endTime, duration);
    }

    @Test
    public void testChannelProgrammeBuilderBuildObjectsShouldBeEqual() {
        ChannelProgramme actual = new ChannelProgramme.Builder()
                .withChannelProgrammeId(channelProgrammeId)
                .withChannel(channel).withProgramme(programme)
                .withEpisode(episode).withRepeat(repeat)
                .withNewSeries(newSeries).withPremiere(premiere)
                .withChoice(choice).withSubtitles(subtitles)
                .withDeafSigned(deafSigned).withStarRating(starRating)
                .withStartTime(startTime)
                .withEndTime(endTime).withDuration(duration).build();
    ...
    public void testChannelProgrammeBuilderBuildObjectsShouldNotBeEqual() {
        ChannelProgramme actual = new ChannelProgramme.Builder()
                .withChannelProgrammeId(1L).build();
