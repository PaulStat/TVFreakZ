...
public class ChannelProgrammeControllerTest {
  ...
  public void testFilterChannelProgrammesWhenChannelListAndGenreListAreEmptyAndProgNameIsTooLong() throws Exception {
    ...
    mockMvc.perform(post("/api/filter")
        ...
        .andExpect(model().attributeHasFieldErrors("filter", "fromDateTime"))
        .andExpect(model().attributeHasFieldErrors("filter", "toDateTime"))
        .andExpect(model().attribute("filter", hasProperty("progName", is(progName))))
        .andExpect(model().attribute("filter", hasProperty("subtitled", is(false))))
        .andExpect(model().attribute("filter", hasProperty("signed", is(false))))
        .andExpect(model().attribute("filter", hasProperty("film", is(false))));

    verifyZeroInteractions(channelProgrammeServiceMock);
  }

  @Test
  public void testFilterChannelProgrammesShouldReturnRedirectionToResults() throws Exception {
  ...
  @Test
  public void testFilterChannelProgrammesFilterRedirectionResults() throws Exception {
    ChannelProgramme[] channelProgrammeShowings = new ChannelProgramme[]{TestUtil.CHANNEL_PROGRAMMES[1]};

    when(channelProgrammeServiceMock.filterChannelProgrammes(any(ProgrammeSearchDTO.class))).thenReturn(Arrays.asList(channelProgrammeShowings));

    String progName = "Aliens";
    String[] channelIdList = new String[]{"1"};
    String[] genreIdList = new String[]{"1"};

    mockMvc.perform(get("/filterResults")
        .param("progName", progName)
        .param("channelIdList", channelIdList)
        .param("genreIdList", genreIdList)
        .param("toDateTime", "24/11/2014")
        ...
        .andExpect(jsonPath("$", hasSize(channelProgrammeShowings.length)))
        .andExpect(jsonPath("$[0].programme.progTitle", is("Aliens")));

    verify(channelProgrammeServiceMock, times(1)).filterChannelProgrammes(any(ProgrammeSearchDTO.class));
