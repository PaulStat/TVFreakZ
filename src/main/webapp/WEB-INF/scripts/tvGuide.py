   ...
   def setupEnvironment(self):
      try:
          ...
          self._cursor.execute("""CREATE TABLE IF NOT EXISTS EPISODE (
                                      EPISODEID INT NOT NULL AUTO_INCREMENT, DIRECTORID INT NOT NULL, SUBTITLE VARCHAR(200),
                                      EPISODE VARCHAR(200), DESCRIPTION TEXT, PRIMARY KEY(EPISODEID), INDEX (DIRECTORID), INDEX(SUBTITLE,EPISODE),
                                      INDEX(SUBTITLE), INDEX(EPISODE), FOREIGN KEY (DIRECTORID) REFERENCES DIRECTOR(DIRECTORID)
                                  ) ENGINE=INNODB""")
   ...
   def updateEpisodes(self):
       try:
          sqlString = """INSERT INTO EPISODE (
                             SUBTITLE, EPISODE, DIRECTORID, DESCRIPTION)
                         SELECT
                             T.SUBTITLE, T.EPISODE, MAX(D.DIRECTORID),
                             MAX(T.DESCRIPTION)
                         FROM
                             TVTEMPTABLE T
